...
def transaction(the_name, the_id, the_account_number): #this is were the transactions, adding and withdrawing, are completed
    ...
    if int(new_user_file[the_length][6].strip("$")) == 0:
        ...
        new_total = int(new_user_file[the_length][6].strip("$"))+the_money
        info_to_write = the_name+str(the_id)+str(the_day)+"/"+str(the_month)+"/"+str(the_year)+" at "+str(the_time)+" $"+str(the_money)+" $"+str(new_total)+" "+str(the_month)+" "+str(attempts)+" "+str(the_month)+" "+str(attempts)
        ...
        if the_kind == "add": #if the user wants to add to his account
            while True:
                ...
                the_time = int(input("Enter the time: "))
                new_total = int(new_user_file[the_length][6].strip("$"))+the_money
                interest_total = round(((int(new_user_file[the_length][6].strip("$"))+the_money)*the_interest)/12, 2) + int(new_user_file[the_length][6].strip("$"))+the_money
            ...
            while True: #making a withdrawal
                ...
                if int(new_user_file[the_length][7]) != the_month:
                    counter = 5
                    if int(new_user_file[the_length][6].strip("$")) - the_money < 0:
                    ...
                    new_money_total = int(new_user_file[the_length][6].strip("$")) - the_money
                    counter -= 1
                    info_to_write = the_name+" "+str(the_id)+" "+str(the_month)+"/"+str(the_day)+"/"+str(the_year)+" at "+str(the_time)+" $"+str(the_money)+" $"+str(new_money_total)+" "+str(the_month)+" "+str(counter)
                    my_new_user_file.write(info_to_write)
                    my_new_user_file.write('\n')
